...
if __name__ == "__main__":
    ...
    all_coordinates = witness_dict["coordinates"]
    max_coords = int(public_dict["max_coordinates"])
    ...
    witness_dict["current_circles_distance_coef"] = "0"
    ...
    init_state_hash = hash_state(
        ...
        [
            ...
            int(witness_dict["current_circles_distance_coef"]),
    ...
    for i in range(iterations):
        witness_dict["coordinates"] = all_coordinates[i * chunk_size : (i + 1) * chunk_size]

        # Hash the current coordinates and put the result into the instance
        int_coords = list(map(lambda x : list(map(int, x)), witness_dict["coordinates"]))
        ...
        os.chdir("../../..")
        result = subprocess.run(["./runrust", "docs/M27-electric-vehicle/ev-inf-mod-multi-part.zksc", json_loc + "public.json", json_loc + "instance.json", json_loc + "witness.json", "-o", "/tmp/fff"], capture_output=True, text=True)
        ...
        for w, word in enumerate(words):
            if word == "Hash:":
                hash_counter += 1
            if hash_counter == 3:
                instance_dict["new_state_hash"] = [words[w + 4 * i] for i in range(1, 6, 1)]
                break
        ...
        for w, word in enumerate(words):
            if word == "Newdistances:":
                witness_dict["current_total_distance_coef"] = words[w + 4]
                witness_dict["current_circles_distance_coef"] = words[w + 8]
                break
        ...
        witness_dict["last_point"] = witness_dict["coordinates"][-1]
